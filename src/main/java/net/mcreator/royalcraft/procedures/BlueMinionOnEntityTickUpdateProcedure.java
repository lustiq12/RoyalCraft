...
public class BlueMinionOnEntityTickUpdateProcedure {
    public static void execute(LevelAccessor world, Entity entity) {
        ...
        if (RoyalcraftModVariables.WorldVariables.get(world).WorldTimer == 1) {
            if (entity instanceof BlueMinionEntity _Minion) {
                _Minion.cachedTarget = lvl.getEntitiesOfClass(LivingEntity.class,
                                new AABB(x - 40, y - 40, z - 40, x + 40, y + 40, z + 40),
                                e -> e.getType().is(TagKey.create(Registries.ENTITY_TYPE, ResourceLocation.tryParse("royalcraft:red"))))
                        .stream()
        ...
        if (entity instanceof BlueMinionEntity _Minion && _Minion.cachedTarget != null) {
            if (entity instanceof Mob _entity) {
